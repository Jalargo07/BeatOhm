...
import com.beatohm.audio.AudioVisualizerManager
...
import com.beatohm.ui.player.PlayerAnimationHelper
import com.beatohm.ui.player.PlayerLayoutManager
import com.beatohm.ui.player.PlayerLyricsHelper
    ...
    private fun observePlaybackState() {
        viewModel.isPlaying.observe(viewLifecycleOwner) { playing ->
            ...
            if (playing) {
                ...
                val service = (requireActivity() as? com.beatohm.MainActivity)?.playbackService
                if (service != null) {
                ...
                WaveformSeekBar.getCachedWaveform(path)
                    ...
                    ?: run {
                        ...
                        try {
                            ...
                            val numBars = com.beatohm.audio.WaveformExtractor.barsForDuration(realDurationMs)
                            val data = com.beatohm.audio.WaveformExtractor.extract(path, numBars)
    ...
    private fun setupControls() {
        ...
        binding.btnLyrics.setOnClickListener {
            ...
            if (lyrics.isBlank()) {
                val path = song.filePath.ifBlank { song.youtubeUrl }
                if (path.isNotBlank()) {
                    lyrics = com.beatohm.data.AudioTagReader.readLyrics(path)
                }
            }
            if (lyrics.isBlank()) {
    ...
    private fun startPositionUpdater() {
        updateRunnable = object : Runnable {
            override fun run() {
                ...
                val activity = b.root.context as? com.beatohm.MainActivity
                val service = activity?.playbackService
